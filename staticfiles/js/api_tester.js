(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const elements = {
            ...
            collectionsActions: root.querySelector('.api-tester__collections-actions'),
            ...
            importPostmanButton: document.getElementById('import-postman'),
    ...
    const scriptPanels = elements.scriptsPanels || [];
        ...
        function resolveSaveModal(value) {
        ...
        const openSaveModal = (defaultName) => {
            ...
            window.requestAnimationFrame(() => {
                elements.saveRequestNameInput.focus();
                elements.saveRequestNameInput.select();
            });
        };

        const promptForRequestName = async (defaultName) => {
            if (!elements.saveRequestModal || !elements.saveRequestNameInput) {
                return window.prompt('Save request as:', defaultName);
            }
            if (saveModalResolver) {
                return null;
            }
        ...
        const getDirectoryBreadcrumb = (collectionId, directoryId) => {
            if (!collectionId || !directoryId) {
                return [];
            }
            const directoryMap = state.directoryMaps.get(collectionId);
            if (!directoryMap) {
                return [];
            }
            const segments = [];
            let current = directoryMap.get(directoryId) || null;
            const guard = new Set();
            while (current) {
                ...
                current = directoryMap.get(current.parent_id) || null;
            }
        ...
        const highlightSelection = () => {
            ...
            cards.forEach((card) => {
                ...
                directoryButtons.forEach((button) => {
                    const isActive = Number(button.dataset.directoryId) === state.selectedDirectoryId;
                    button.classList.toggle('active', isActive);
                });
            });
        ...
        const refreshCollections = async ({
            ...
            if (nextCollectionId !== null) {
                activeCollection = state.collections.find((item) => item.id === nextCollectionId) || null;
            }
            ...
            if (nextRequestId === null) {
                if (preserveSelection && previousRequestId && activeCollection.requests.some((item) => item.id === previousRequestId)) {
                    nextRequestId = previousRequestId;
                } else {
                    nextRequestId = activeCollection.requests[0]?.id ?? null;
                }
            }
            ...
            if (nextDirectoryId === null) {
                if (state.selectedRequestId) {
                    const matchingRequest = activeCollection.requests.find((item) => item.id === state.selectedRequestId);
                    nextDirectoryId = matchingRequest?.directory_id ?? null;
                } else if (preserveSelection && previousDirectoryId && state.directoryMaps.get(activeCollection.id)?.has(previousDirectoryId)) {
            ...
            const selectedRequest = state.selectedRequestId
                ? activeCollection.requests.find((item) => item.id === state.selectedRequestId) || null
                : null;
            if (!selectedRequest && activeCollection.requests.length === 0) {
                state.selectedRequestId = null;
            }

            renderEnvironmentOptions(activeCollection);
            populateForm(activeCollection, selectedRequest);
            highlightSelection();
        ...
        const renderCollections = (filterText = '') => {
            ...
            filtered.forEach((collection) => {
                const sortedRequests = Array.isArray(collection.requests)
                    ? [...collection.requests].sort((a, b) => (a.order ?? 0) - (b.order ?? 0) || a.name.localeCompare(b.name))
                    : [];
                const directories = Array.isArray(collection.directories)
                    ...
                    children.forEach((directory) => {
                        ...
                        button.addEventListener('click', (event) => {
                            ...
                            if (isRequestInDirectorySubtree(state.selectedRequestId, directory.id)) {
                                nextRequest = sortedRequests.find((req) => req.id === state.selectedRequestId) || null;
                            }
                            if (!nextRequest) {
                            ...
                            renderEnvironmentOptions(collection);
                            populateForm(collection, nextRequest);
                            highlightSelection();
                        });
                ...
                card.addEventListener('click', () => {
                    ...
                    renderEnvironmentOptions(collection);
                    populateForm(collection, firstRequest);
                    highlightSelection();
                });
        ...
        const buildRequestDefinition = ({ name, collectionId, directoryId }) => {
        ...
        const bootstrap = async () => {
            try {
                ...
                if (state.selectedCollectionId && state.selectedRequestId) {
                    ...
                    setStatus('Collection loaded. Select or create a request to begin.', 'neutral');
                } else {
                    setStatus('No collections available yet.', 'neutral');
                }
            } catch (error) {
        ...
        if (elements.saveRequestButton) {
            elements.saveRequestButton.addEventListener('click', async () => {
                ...
                let directoryId = state.selectedDirectoryId;
                if (directoryId === null && existingRequest?.directory_id) {
                    directoryId = existingRequest.directory_id;
                }
                ...
                try {
                    ...
                    if (existingRequest && directoryId === null && existingRequest.directory_id) {
                ...
                const requestsEndpoint = endpoints.requests.endsWith('/') ? endpoints.requests : `${endpoints.requests}/`;
                ...
                try {
                    ...
                    const savedDirectoryId = response?.directory_id ?? directoryId ?? null;
                    await refreshCollections({
                        preserveSelection: false,
                        focusCollectionId: collection.id,
                        focusRequestId: savedRequestId,
                        focusDirectoryId: savedDirectoryId,
                    });
